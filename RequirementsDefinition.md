...
usecaseDiagram
    actor Müşteri
    actor "Menü Yöneticisi" as MenuManager
    actor "Kafeterya Personeli" as CafeteriaStaff
    actor "Yemek Dağıtımcısı" as Delivery
    actor Banka
    actor Sistem

    Müşteri --> (Yemek Siparişi Verir)
    Müşteri --> (Yemek Siparişini Değiştirir)
    Müşteri --> (Yemek Siparişini İptal Eder)
    Müşteri --> (Menüyü Görüntüler)

    (Yemek Siparişi Verir) ..> (Ödemesini Yapabilir) : <<include>>
    (Yemek Siparişini Değiştirir) ..> (Ödemesini Yapabilir) : <<include>>
    (Yemek Siparişini İptal Eder) ..> (İptal edilen siparişlerin geri ödemesi yapılır) : <<include>>

    (Ödemesini Yapabilir) --> Banka
    (İptal edilen siparişlerin geri ödemesi yapılır) --> Sistem

    MenuManager --> (Menüyü Oluşturabilir)
    MenuManager --> (Menüleri Arşivleyebilir)
    MenuManager --> (Yemek Spesiyali Tanımlayabilir)

    CafeteriaStaff --> (Yemek Teslimatı Talep Edebilir)
    CafeteriaStaff --> (Raporlar Oluşturabilir)

    Delivery --> (Yemek Teslimatını Kaydedebilir)
    Delivery --> (Teslimat Talimatlarını Yazdırabilir)
```
